...
export const Body: React.FC<{selectedKey: string}> = memo(({ selectedKey }) => {
    ...
    const transKuizu = async (dataName:string, startItem: number, perItem: number, numOfNormalChoices: number)=>{
        const data = await useGetJsonData(dataName, startItem, perItem)
        setSection("kuizu");
        setKuizu({data, numOfNormalChoices})
    }
            ...
            {Array.from({ length: stageData.total }).map((_, i) => {
                ...
                return (
                    <div className="block" key={i}>
                        ...
                        <button className="button1" onClick = {() => transKuizu(dataName, start, perItem, NumOfChoice)}>学習</button>
